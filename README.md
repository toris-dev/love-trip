...
- **상태**: 구현 완료
- **설명**: 프리미엄 사용자를 위한 일정 최적화 (거리 기반 Nearest Neighbor)
- **API**: `POST /api/travel-plans/[id]/reschedule`
- **기능**: 여행 상세 페이지에서 "일정 자동 재편성" 버튼으로 일차별 방문 순서 최적화

### 5. 예산 최적화 대안 코스 ✅

- **상태**: 구현 완료
- **설명**: 예산 초과 시 프리미엄 사용자에게 예산 맞춤 대안 장소 추천
- **API**: `GET /api/travel-plans/[id]/budget/optimize` (프리미엄 시 `alternativePlaces` 포함)
- **기능**: 예산 최적화 카드 내 "예산 맞춤 대안 장소" 섹션으로 지역·가격대 필터 추천
...
- `anniversary_reminders`: 기념일 알림 (푸시 알림 연동)
...
- AI 일정 재편성 (프리미엄, 거리 기반 일차별 순서 최적화)
- 예산 최적화 대안 코스 (프리미엄, 예산 맞춤 대안 장소 추천)
- 기념일 알림 (anniversary_reminders 테이블, 푸시 알림 발송 API `/api/cron/anniversary-reminders`)
