...
import type { CalendarEvent, SharedCalendar, Couple } from "../types"
...
export class CalendarService {
  ...
  async requestCouple(nickname: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error: errorData.error || "사용자를 찾을 수 없습니다. 닉네임을 확인해주세요.",
        }
      }
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "커플 요청에 실패했습니다",
      }
    }
  }
  ...
  // 이벤트 생성
  async createEvent(
    event: Omit<CalendarEvent, "id" | "created_at" | "updated_at" | "created_by">
  ...
  async updateEvent(
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "이벤트 업데이트에 실패했습니다",
      }
    }
  }
