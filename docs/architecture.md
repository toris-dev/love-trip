# 🏗 LOVETRIP 아키텍처 문서

## 📋 목차

- [시스템 개요](#시스템-개요)
- [아키텍처 원칙](#아키텍처-원칙)
- [모노레포 구조](#모노레포-구조)
- [레이어 구조](#레이어-구조)
- [데이터 흐름](#데이터-흐름)
- [기술 스택](#기술-스택)

## 🎯 시스템 개요

LOVETRIP은 **모노레포(Monorepo)** 구조를 사용하는 Next.js 기반 풀스택 애플리케이션입니다.

### 핵심 설계 원칙

1. **도메인 주도 설계 (DDD)**: 비즈니스 도메인별로 패키지 분리
2. **관심사의 분리**: UI, 비즈니스 로직, 데이터 접근 계층 분리
3. **재사용성**: 공통 컴포넌트와 유틸리티를 패키지로 추출
4. **타입 안정성**: TypeScript로 엔드투엔드 타입 안정성 보장

## 🎨 아키텍처 원칙

### 1. 모노레포 구조

```
love-trip/
├── apps/
│   └── web/              # Next.js 애플리케이션
├── packages/
│   ├── ui/               # UI 컴포넌트 라이브러리
│   ├── shared/           # 공통 타입 및 유틸리티
│   ├── api/              # API 클라이언트
│   ├── user/             # 사용자 도메인
│   ├── couple/           # 커플 도메인
│   ├── planner/          # 여행 계획 도메인
│   ├── recommendation/   # 추천 도메인
│   ├── expense/          # 경비 도메인
│   ├── gamification/     # 게이미피케이션 도메인
│   └── subscription/     # 구독 도메인
└── pnpm-workspace.yaml
```

### 2. 레이어 구조

```
┌─────────────────────────────────────┐
│   Presentation Layer (apps/web)     │
│   - Pages, Components, Hooks         │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│   Domain Layer (packages/*)         │
│   - Business Logic, Services        │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│   Data Layer (packages/api)         │
│   - Supabase Client, API Routes     │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│   Infrastructure (Supabase)         │
│   - Database, Auth, Storage          │
└─────────────────────────────────────┘
```

## 📦 모노레포 구조

### apps/web

Next.js 15 기반 웹 애플리케이션

**구조**:

```
apps/web/
├── src/
│   ├── app/              # App Router 페이지
│   ├── components/      # React 컴포넌트
│   │   ├── features/    # 기능별 컴포넌트
│   │   ├── layout/      # 레이아웃 컴포넌트
│   │   └── shared/      # 공유 컴포넌트
│   └── hooks/           # 커스텀 훅
```

**책임**:

- UI 렌더링
- 사용자 인터랙션 처리
- 라우팅
- 클라이언트 사이드 상태 관리

### packages/ui

재사용 가능한 UI 컴포넌트 라이브러리

**책임**:

- 디자인 시스템 컴포넌트
- 스타일 토큰
- 공통 UI 패턴

**사용 예시**:

```typescript
import { Button, Card } from "@lovetrip/ui/components"
```

### packages/shared

공통 타입, 유틸리티, 상수

**책임**:

- TypeScript 타입 정의
- 공통 유틸리티 함수
- 상수 값

**사용 예시**:

```typescript
import type { Place, TravelPlan } from "@lovetrip/shared/types"
import { formatDate } from "@lovetrip/shared/utils"
```

### packages/api

API 클라이언트 및 서버 사이드 로직

**구조**:

```
packages/api/
├── supabase/
│   ├── client.ts        # 브라우저 클라이언트
│   └── server.ts        # 서버 클라이언트
└── clients/             # 외부 API 클라이언트
```

**책임**:

- Supabase 클라이언트 생성
- 외부 API 통신
- 데이터 변환

### packages/\* (도메인 패키지)

비즈니스 도메인별 패키지

**패턴**:

```
packages/{domain}/
├── services/            # 비즈니스 로직
├── components/          # 도메인 특화 컴포넌트
├── hooks/              # 도메인 특화 훅
└── types/              # 도메인 타입
```

**예시**:

- `packages/planner/`: 여행 계획 로직
  - `place-service.ts`: 장소 서비스 (하이브리드 데이터 모델)
  - `travel-service.ts`: 여행 계획 서비스
  - `user-course-service.ts`: 사용자 코스 서비스
- `packages/expense/`: 경비 관리 로직
- `packages/recommendation/`: 추천 알고리즘
- `packages/api/`: API 클라이언트
  - `clients/`: 외부 API 클라이언트 (네이버, Tour API)
  - `supabase/`: Supabase 클라이언트 및 트랜잭션 관리

## 🔄 데이터 흐름

### 1. 클라이언트 → 서버

```
User Action
  ↓
React Component
  ↓
Custom Hook / Service
  ↓
API Route (Next.js)
  ↓
Supabase Client
  ↓
Database
```

### 2. 서버 → 클라이언트

```
Database
  ↓
Supabase (Realtime)
  ↓
API Route
  ↓
React Component
  ↓
UI Update
```

### 3. 상태 관리

- **서버 상태**: React Query (예정) 또는 SWR
- **클라이언트 상태**: React Context / Zustand (필요시)
- **폼 상태**: React Hook Form

## 🛠 기술 스택

### Frontend

- **Framework**: Next.js 15.2.4
- **UI Library**: React 19.2.1
- **Language**: TypeScript 5.7
- **Styling**: Tailwind CSS
- **Components**: Radix UI
- **Animation**: Framer Motion
- **Forms**: React Hook Form + Zod

### Backend

- **BaaS**: Supabase
  - PostgreSQL Database
  - Authentication
  - Row Level Security (RLS)
  - Realtime Subscriptions
  - Storage

### External APIs

- **Tour API**: 한국관광공사 관광 정보
  - 클라이언트: `packages/api/clients/tour-api-client.ts`
  - 지역별 관광 정보 조회
  - 키워드 검색
- **네이버 지도 API**: 지도 및 장소 검색
  - 클라이언트: `packages/api/clients/naver-api-client.ts`
  - Places API: 장소 검색
  - Geocoding API: 주소 검색

### 개발 도구

- **Package Manager**: pnpm (workspace)
- **Testing**: Vitest, Testing Library
- **Linting**: ESLint
- **Formatting**: Prettier
- **Type Checking**: TypeScript

## 🔐 보안 아키텍처

### 인증

- Supabase Auth 사용
- JWT 기반 세션 관리
- Row Level Security (RLS)로 데이터 접근 제어

### 데이터 보안

- 모든 테이블에 RLS 정책 적용
- 사용자는 자신의 데이터만 접근 가능
- 커플 간 데이터 공유는 명시적 권한 필요

### API 보안

- 환경 변수로 민감 정보 관리
- 서버 사이드에서만 Service Role Key 사용
- 클라이언트는 Anon Key만 사용

## 📊 성능 최적화

### 클라이언트

- Next.js Image 최적화
- 코드 스플리팅
- 동적 임포트 (Dynamic Import)
- React Server Components 활용

### 서버

- Edge Functions (필요시)
- ISR (Incremental Static Regeneration)
- 데이터베이스 인덱싱
- 쿼리 최적화

## 🔄 배포 아키텍처

### 개발 환경

```
Local Development
  ↓
Next.js Dev Server
  ↓
Supabase (Development)
```

### 프로덕션 환경

```
Vercel (예정)
  ↓
Next.js Production Build
  ↓
Supabase (Production)
  ↓
CDN (Static Assets)
```

## 📝 관련 문서

- [프로젝트 기획서](./project-plan.md)
- [API 명세서](./api-spec.md)
- [개발 가이드](./development-guide.md)
- [아키텍처 결정 기록](./decisions/README.md)
